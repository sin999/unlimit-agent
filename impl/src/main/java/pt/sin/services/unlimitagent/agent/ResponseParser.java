...
public class ResponseParser {
    ...
    public void validateAnalysis(IncidentAnalysis analysis) {
        ...
        if (isBlank(analysis.getCategory())) {
            errors.add("'category' is blank or missing");
        }
        if (isBlank(analysis.getSummary())) {
            errors.add("'summary' is blank or missing");
        }
        if (isNull(analysis.getSeverity())) {
            errors.add("'severity' is missing");
        }
        if (ofNullable(analysis.getHypotheses()).map(Collection::isEmpty).orElse(true)) {
            errors.add("'hypotheses' is empty or missing");
        } else {
            if (analysis.getHypotheses().size() > MAX_HYPOTHESES) {
                errors.add("'hypotheses' contains more than " + MAX_HYPOTHESES + " items (" + analysis.getHypotheses().size() + ")");
            }
            for (int i = 0; i < analysis.getHypotheses().size(); i++) {
                Hypothesis h = analysis.getHypotheses().get(i);
                if (h.getTitle() == null || h.getTitle().isBlank()) {
                    errors.add("hypotheses[" + i + "].title is blank");
                }
                if (h.getReasoning() == null || h.getReasoning().isBlank()) {
                    errors.add("hypotheses[" + i + "].reasoning is blank");
                }
                int steps = (h.getNextSteps() == null) ? 0 : h.getNextSteps().size();
