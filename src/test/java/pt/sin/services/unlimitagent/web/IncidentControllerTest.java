...
class IncidentControllerTest {
    ...
    @MockitoBean IncidentAgentPipeline pipeline;

    private static final IncidentAnalysis VALID_ANALYSIS = new IncidentAnalysis(
            "External payment provider issue",
            "PayGate is not responding.",
            Severity.HIGH,
            List.of(new Hypothesis("PayGate degradation", "Timeouts only on PayGate.",
                    List.of("Check status page", "Review logs"))));

    // ── C-1 ─────────────────────────────────────────────────────────────────
    @Test
    void validRequest_returns200WithCategory() throws Exception {
        when(pipeline.analyze(any())).thenReturn(VALID_ANALYSIS);
    ...
    void parseException_returns502WithParseError() throws Exception {
        when(pipeline.analyze(any())).thenThrow(new ResponseParseException("bad json"));
